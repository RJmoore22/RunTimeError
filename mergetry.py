...
def bubbleSort(yval): 
    ...
    canvas.create_text(Window_Width / 2, 40, text=f"Traced Memory (Current, Peak): {tracemalloc.get_traced_memory()} Kibibyte ", fill = "white")
    ...
    if len(yval) > 1:
        ...
        while i < len(L) and j < len(R):
            if L[i] <= R[j]:
                ...
                canvas.update()
                objects[k] =canvas.create_rectangle(despos[k],basey,colwidth, yval[k], fill = color2, outline = "")
                canvas.update()
                i += 1
            else:
                yval[k] = R[j]

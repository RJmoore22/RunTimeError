...
def animate_rect():
    ...
    canvas.update()
    #start = yval.index(min(yval))
    #end = yval.index(max(yval))
...
def partition(yval, start, end):
    ...
    for j in range( start, end):
        if yval[j] <= pivot:
            ...
            canvas.itemconfig(objects[i], fill = "Blue")
            canvas.update()
            canvas.delete(objects[i])
            canvas.update()
            canvas.itemconfig(objects[i], fill = "Blue")
            canvas.update()
            canvas.delete(objects[j])
            canvas.update()
            
            objects[i] = canvas.create_rectangle(despos[i],basey,despos[i] + colwidth, yval[i], fill = "Red", outline = "")
            canvas.update()
            
            canvas.itemconfig(objects[i], fill = "Red")
            canvas.update()
            objects[j] = canvas.create_rectangle(despos[j],basey,despos[j] + colwidth, yval[j], fill = "Red", outline = "")
            canvas.update()
            canvas.itemconfig(objects[i], fill = "Red")
    ...
    canvas.update()
    canvas.delete(objects[end])
    canvas.update()
    objects[i + 1] = canvas.create_rectangle(despos[i + 1],basey,despos[i + 1] + colwidth, yval[i + 1], fill = "Red", outline = "")
    canvas.update()
    objects[end] = canvas.create_rectangle(despos[end],basey,despos[end] + colwidth, yval[end], fill = "Red", outline = "")
